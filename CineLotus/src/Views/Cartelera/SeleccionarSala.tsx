import { Container, CircularProgress, Alert, Box, Typography, Card, 
...
const SeleccionarSala = () => {
    ...
    useEffect(() => {
      ...
      axios
      .get(`http://localhost:3000/reservas/step2/${id}/${fecha}`, {
        headers: {
        ...
        <Card sx={{ display: 'flex', marginBottom: 2, width: '100%' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
            <CardContent>
              ...
              <Typography color="text.secondary">{peli?.descripcionPelicula}</Typography>
            </CardContent>
          </Box>
        </Card>
        <Box
          sx={{
            display: 'grid',
            gridTemplateColumns: { xs: '1fr 1fr', sm: '1fr 1fr 1fr', lg: '1fr 1fr 1fr 1fr 1fr 1fr' },
            gap: 2,
            marginTop: 2
          }}
        >
          {peliculas.map((pelicula) => (
            <SalaCard key={pelicula.id} id={pelicula.id} nombreSala={pelicula.nombreSala}/>
          ))}
        </Box>
      </Container>
    );
